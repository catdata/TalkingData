...
setwd("C:/Users/usuario/Desktop/Kaggle TalkingData Mobile User Demographics/DADES")

library(data.table)

label_train <- fread("gender_age_train.csv", colClasses=c("character", "character", "integer", "character"))
label_test <- fread("gender_age_test.csv", colClasses=c("character"))
...
brand <- fread("phone_brand_device_model.csv", colClasses=c("character", "character", "character"))
...
rm(events, event_app, event_apps)
gc()
...
rm(device_event_apps, f_split_paste)
gc()
...
write.csv(res_submit, file="submit.csv", row.names=F, quote=F)
